...
export function UserDashboard({ initialUser }: { initialUser: UserData }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-stone-900 mb-2">Log ud</h2>
        <p className="text-sm text-stone-500 mb-4">
          Afslut din session på denne enhed.
        </p>
        <button
          onClick={() => signOut({ callbackUrl: "/" })}
          className="rounded-xl border border-stone-200 px-4 py-2 text-sm text-stone-700 hover:bg-stone-50 transition-colors"
        >
          Log ud
        </button>
      </section>
